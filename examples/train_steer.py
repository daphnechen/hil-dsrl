...
flags.DEFINE_float("bc_timestep_decay", 0.0001, "bc timestep decay coefficient (e.g., 1e-4, 5e-4, 1e-3)")
...
def actor(agent, data_store, intvn_data_store, env, sampling_rng, bc_data_store = None, use_bc_loss = False):
    """
    ...
    if FLAGS.eval_checkpoint_step:
        ...
        filei = 0
        os.makedirs(os.path.join(FLAGS.checkpoint_path, "evals"), exist_ok=True)
        while os.path.exists(os.path.join(FLAGS.checkpoint_path, "evals", f"eval_traj_{FLAGS.eval_checkpoint_step}_{filei}.pkl")):
            filei += 1
        fp = os.path.join(FLAGS.checkpoint_path, "evals", f"eval_traj_{FLAGS.eval_checkpoint_step}_{filei}.pkl")
    ...
    for step in pbar:
        ...
        with timer.context("step_env"):
            ...
            if use_bc_loss and "intervene_action" in info:
            ...
            if done or truncated:
                ...
                info["episode"]["total_intervention_steps"] = total_intervention_steps
                info["episode"]["intervention_rate"] = total_interventions / (step + 1)
                info["episode"]["intervention_rate_steps"] = total_intervention_steps / (step + 1)
...
def learner(rng, agent: SACAgentHybridSingleArm, replay_buffer, demo_buffer, bc_buffer = None, wandb_logger=None, use_bc_loss: bool = False, include_timestep: bool = False):
    """
    ...
    if bc_buffer is not None:
        server.register_data_store("actor_env_bc", bc_buffer)
    ...
    for step in tqdm.tqdm(
        ...
        # Convert training step to environment step for bc_weights age calculation
        env_step = len(replay_buffer)
        if env_step >= env_step_threshold:
            env_step_threshold += 200
            print_green(f"Environment steps reached: {env_step}")
...
def main(_):
    ...
    if FLAGS.learner:
        ...
        learner(
            ...
            bc_buffer=bc_buffer,
            use_bc_loss=use_bc_loss,
            include_timestep=True if FLAGS.method != "hil" else False,
        )
        ...
        bc_data_store = QueuedDataStore(10000) if use_bc_loss else None
