...
flags.DEFINE_string("flow_policy_path", None, "Path to load diffusion checkpoints.")
flags.DEFINE_float("noise_scale", 3.0, "Noise scale for diffusion policy during evaluation.")
    ...
    for step in pbar:
        ...
        with timer.context("sample_actions"):
            if step < config.random_steps:
                ...
                sampling_rng, key = jax.random.split(sampling_rng)
                actions = jax.random.normal(sampling_rng, shape=env.action_space.shape)
                actions = np.asarray(jax.device_get(actions)) * FLAGS.noise_scale
            else:
                sampling_rng, key = jax.random.split(sampling_rng)
                actions = agent.sample_actions(
                    observations=utils.device_put(obs),
        ...
        with timer.context("step_env"):
            ...
            if "intervene_action" in info:
                ...
                info["policy_action"] = actions
                actions = info["intervene_action"]
                actions = torch.tensor(actions, device=device).unsqueeze(0)
                noisy_actions = noise_scheduler.add_noise(
                    actions, noise, timesteps)
                actions = noisy_actions.squeeze(0).cpu().numpy()
            ...
            timesteps = torch.randint(
                noise_scheduler.config.num_train_timesteps-1, noise_scheduler.config.num_train_timesteps,
                (1,), device=device
            ).long()
...
def main(_):
    ...
    assert os.path.exists(FLAGS.flow_policy_path), "Diffusion checkpoint path does not exist."
    from diffusion_policy.policy import DiffusionPolicy
    import os
    assert os.path.exists(FLAGS.flow_policy_path), "Diffusion checkpoint path does not exist."
    assert os.getenv('FLOW_MATCHING', '0') == '1'
    
    base_dp_policy = DiffusionPolicy.load(FLAGS.flow_policy_path)
    print_green(f"Loaded diffusion policy from {FLAGS.flow_policy_path}.")
    ...
    def create_replay_buffer_and_wandb_logger():
        ...
        wandb_logger = make_wandb_logger(
            project="hil-serl-cubereach-dsrl",
