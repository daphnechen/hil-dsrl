...
class EnvConfig(DefaultEnvConfig):
    ...
    # {"pose":[0.4889125134308501,-0.03112635953638949,0.1321097352945601,0.7611749148040746,0.6483143343976019,-0.012088473554233627,-0.01245558889546912]}
    RESET_POSE = np.array([0.49, -0.03, 0.13, np.pi, 0, np.pi / 2])

    # Step 2: add bounding boxes
    # {"pose":[0.41307143187543544,-0.214872149182247,0.1689512852455045,0.7543336601715125,0.6530622917458934,-0.06624099616440385,0.010124359683360937]}
    # {"pose":[0.5841186646474222,0.1218348168493024,0.10117991290504835,0.7230574342225558,0.6905986335474248,-0.0055205194611559726,-0.015198619108434232]}
    # 0.4772689429100415,-0.12894117193291577
    ABS_POSE_LIMIT_LOW  = np.array([0.42, -0.18, 0.03, np.pi - 0.05, -0.05, np.pi / 2 - 0.05])
    ABS_POSE_LIMIT_HIGH = np.array([0.59,  0.18, 0.15, np.pi + 0.05,  0.05, np.pi / 2 + 0.05])
...
class TrainConfig(DefaultTrainingConfig):
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False, eval_env=False):
        ...
        if not fake_env:
            env = SpacemouseIntervention(env, eval_env)
        ...
        if classifier:
            ...
            def reward_func(obs):
                ...
                if prob > 0.1:
                    print(f"{prob:.2f}")
                return int(prob > 0.95) # and obs['state'][0, 3] < 0.12
                # return 0.0
